...
#=======================================================================================================================
#=======================================================================================================================
class ScaledDotProductAttention_Layer(tf.keras.layers.Layer):
    ...
    def __init__(self,
        ...
        """
        super(ScaledDotProductAttention_Layer, self).__init__(**kwargs)
    ...
    def get_config(self):
        ...
        base_config = super(ScaledDotProductAttention_Layer, self).get_config()
...
class MultiHeadAttention_Layer(tf.keras.layers.Layer):
    ...
    def get_config(self):
        ...
        base_config = super(MultiHeadAttention_Layer, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
    ...
    def build(self, input_shape):
        ...
        super(MultiHeadAttention_Layer, self).build(input_shape)
    ...
    def call(self, inputs, mask=None):
        ...
        scaled_dot_product_attention = ScaledDotProductAttention_Layer(
            history_only=self.history_only,
...
def Encoder(x):
    NUM_FEEDBACK_BITS = 30
    EMBEDDING_DIM = 128
    NUM_HEAD = 4
    def linear_embedding(y, embedding_dim):
    ...
    def att_block(y, num_head):
        ...
        y, v = MultiHeadAttention_Layer(head_num=num_head)(y)
    ...
    def fc_block(y, inner_dim, embedding_dim):
        ...
        y = layers.Add()([short_cut, y])
        return y
    x = layers.Dense(15*4)(x)
    x = layers.LeakyReLU(alpha=0.2)(x)
    x = layers.Reshape((15,4))(x)
    #x = layers.LeakyReLU(alpha=0.2)(x)
    #x = layers.Dense(401*6)(x)
    #x = layers.LeakyReLU(alpha=0.2)(x)
    #x = layers.Dense(401*36)(x)
    #x = layers.LeakyReLU(alpha=0.2)(x)
    #x = layers.Reshape((401,36))(x)
    
    x = linear_embedding(x, EMBEDDING_DIM)
    ...
    for i in range(6):
        ...
        #iEMBEDDING_DIM = EMBEDDING_DIM*2
        #x = layers.Dense(EMBEDDING_DIM)(x)

    x = layers.LayerNormalization()(x)
    x = layers.Flatten()(x)
    x = layers.Dense(15*16)(x)
    x = layers.LeakyReLU(alpha=0.2)(x)
    x = layers.Dense(15*4,activation='sigmoid')(x)
    #x = layers.Flatten()(x)
...
def Decoder(x):
    ...
    #x = layers.Dense(13* 16)(x)
    #label = layers.Reshape((1,3))(label)
    #label = tf.tile(label,[1,36,1])
    #label = layers.Dense(60)(label)

    #label = layers.Reshape((36,3))
    x = layers.Dense(60)(x)
    x = layers.LeakyReLU(alpha=0.2)(x)
    x = layers.Reshape((15, 4))(x)
...
def get_custom_objects():
    return {"QuantizationLayer":QuantizationLayer,"DeuantizationLayer":DeuantizationLayer,"ScaledDotProductAttention_Layer":ScaledDotProductAttention_Layer, "MultiHeadAttention_Layer":MultiHeadAttention_Layer}
#=======================================================================================================================
#=======================================================================================================================
