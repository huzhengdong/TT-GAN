...
def data_processing():
    print('Data Loading...')
    data_train = h5py.File('input_coeff/coeff_10000_d1.mat', 'r')#Channel_set
  ...
  def on_epoch_end(self, epoch, logs={}):
    if( epoch % 1000 == 0):
      ...
      #self.model.stop_training = True
        self.model.generator.save(os.path.join('epoch/', 'd1_10000e_epoch_{}_gloss_{}.h'.format(epoch,logs['g_loss'])))
        self.model.discriminator.save(os.path.join('epoch/', 'd1_10000e_epoch_discriminator_epoch_{}_dloss_{}.h'.format(epoch,logs['d_loss'])))
...
wgan = WGAN(discriminator=d_model,generator=g_model,latent_dim=latent,discriminator_extra_steps=3,)
...
wgan.generator.save('d1_coeff_generator_10000e_tgan_pdap_latent=128_r.h')

#channel_save = wgan.plot()
print(history.history.keys())
epochs=range(len(history.history['d_loss']))
io.savemat('loss_10000e_pdap.mat',mdict={'epoch':epochs,'d_loss':history.history['d_loss'],'g_loss':history.history['g_loss']})
...
plt.legend()
plt.savefig('loss_d1_10000.jpg')
wgan.discriminator.save('d1_coeff_discriminator_10000e_tgan_pdap_r.h')
